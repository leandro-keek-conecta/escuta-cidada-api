...
export async function routerProjeto(app: FastifyInstance) {
  ...
  app.get("/list/user/:id", {preHandler: AuthMiddleware.required,handler: ProjetoController.listByUserId.bind(ProjetoController)});
  app.get("/list/:id", {preHandler: AuthMiddleware.required,handler: ProjetoController.listByProjetoId.bind(ProjetoController)});
